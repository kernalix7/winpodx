...
REM media_monitor is launched per session via the WinpodxMedia HKCU\Run entry
REM (registered above with WinpodxAgent). That fires for every interactive
REM logon, including RemoteApp sessions, so each session gets its own instance
REM (deduped by media_monitor's per-session mutex). No extra scheduled task is
REM registered here -- an AtLogOn-task experiment hung install.bat (#613) and
REM HKCU\Run already covers the sessions.
