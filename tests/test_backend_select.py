...
* Each branch of :func:`choose_backend` (prefer / auto-podman / podman too old
  → fall through / docker / libvirt / nothing → recommended podman fallback).
...
def _deps(*, podman: bool, docker: bool, virsh: bool) -> dict[str, DepCheck]:
    ...
    return {
        ...
        "virsh": DepCheck(name="virsh", found=virsh, note="libvirt backend"),
...
def test_prefer_explicit_returns_unchanged() -> None:
    for backend in ("podman", "docker", "libvirt", "manual"):
        assert (
            choose_backend(prefer=backend, deps=_deps(podman=True, docker=True, virsh=True))
            == backend
        )
...
def test_prefer_unknown_raises() -> None:
    ...
    with pytest.raises(ValueError, match="unknown backend"):
        choose_backend(prefer="qemu", deps=_deps(podman=True, docker=False, virsh=False))
...
def test_auto_picks_podman_when_present_and_modern() -> None:
    with patch("winpodx.backend.select.podman_major_version", return_value=5):
        assert choose_backend(deps=_deps(podman=True, docker=True, virsh=True)) == "podman"
...
def test_auto_skips_podman_too_old_falls_to_docker() -> None:
    ...
    with patch("winpodx.backend.select.podman_major_version", return_value=3):
        assert choose_backend(deps=_deps(podman=True, docker=True, virsh=False)) == "docker"


def test_auto_skips_podman_too_old_falls_to_libvirt_when_no_docker() -> None:
    with patch("winpodx.backend.select.podman_major_version", return_value=3):
        assert choose_backend(deps=_deps(podman=True, docker=False, virsh=True)) == "libvirt"


def test_auto_picks_docker_when_no_podman() -> None:
    assert choose_backend(deps=_deps(podman=False, docker=True, virsh=True)) == "docker"


def test_auto_picks_libvirt_when_only_one_present() -> None:
    assert choose_backend(deps=_deps(podman=False, docker=False, virsh=True)) == "libvirt"
...
def test_auto_falls_back_to_podman_when_nothing_usable() -> None:
    ...
    assert choose_backend(deps=_deps(podman=False, docker=False, virsh=False)) == "podman"
...
def test_podman_major_version_threshold_is_inclusive() -> None:
    ...
    with patch(
        ...
        assert choose_backend(deps=_deps(podman=True, docker=True, virsh=False)) == "podman"
...
def test_install_sh_mirror_walks_same_priority_order() -> None:
    ...
    # `for candidate in podman docker libvirt; do` is the Automatic-mode loop;
    # it MUST list AUTO_PRIORITY in the same order so the two sources can't
    # drift on which backend wins on a multi-runtime host.
    match = re.search(
