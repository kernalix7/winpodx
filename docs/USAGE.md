...
| `max` | Everything in `balanced` **plus** emulated virtual hardware — disk → SATA (AHCI), GPU → std VGA, no virtio-rng — and `HV=N`. This removes the virtio disk (`vioscsi`/`viostor`) and QXL (`VEN_1B36`) signatures. (The NIC stays virtio: dockur appends `host_mtu=` to the adapter, which only virtio-net supports, so an emulated e1000 aborts QEMU at boot — `netkvm` / the NIC `VEN_1AF4` therefore remain.) **Requires a wipe + reinstall** (changing the boot-disk controller makes the existing install unbootable), so switching into/out of `max` prompts a strong confirmation and reinstalls Windows from scratch. | **Significantly slower** — emulated disk has much lower throughput than virtio, and Hyper-V enlightenments are off |
