...
- **통합된 `winpodx uninstall` — apt 스타일 preview + sudo 패키지 prompt (#255 PR 3).** 사용자 data dir 만 쓸어내던 #255 이전 stub 대체. `uninstall.sh` 와 parity 도달: tray/GUI pkill, reverse-open listener stop, 컨테이너 stop (`--purge` 시에만 rm), desktop entries + 아이콘 + data dir + runtime dir + autostart entry + curl install dir (`~/.local/bin/winpodx-app/`) 제거. `--purge` 가 추가로 podman volume rm + storage path wipe + config dir rm (`apt purge` 시맨틱; 기본 `winpodx uninstall` 은 `apt remove` 시맨틱). 사용자가 정확히 무엇이 제거되는지 보도록 confirm prompt 전 apt 스타일 preview 블록 (`--purge` 시 ~50GB Windows 디스크 경고 포함). 패키지 매니저 install 감지된 시스템에선 끝에 `Also remove the system package via sudo? [y/N]:` prompt — 사용자 수락시 대응 `sudo apt remove` / `sudo dnf remove` / `sudo pacman -Rns` exec. 신규 플래그: `--yes` (스크립트/CI 용 confirm skip), `--no-package-prompt` (패키징 postrm 후크 용).
