...
- 새 **License** GUI 탭. MIT 라이선스 본문 + "Third-party components" 섹션 (winpodx 가 의존하거나 ship 하는 upstream 프로젝트 목록: dockur, FreeRDP, PySide6/Qt, rdprrap, rcedit, Catppuccin) 과 각자의 라이선스 명시. LICENSE 파일은 `bundle_dir()` 로 해소해서 모든 설치 모드 (소스 체크아웃, wheel, FHS, `curl | bash`) 에서 렌더링됨.
