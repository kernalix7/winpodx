...
- **OEM `install.bat` 의 powercfg + PlatformAoAcOverride 블록 제거.** PR #219 (v0.5.5) 가 Modern Standby 가 Windows 게스트의 virtio NIC 를 suspend 하는 것 방지 목적으로 모든 powercfg timeout 을 0 으로 만들고 `PlatformAoAcOverride=0` 강제하는 22 줄 추가. 그 블록이 막으려 했던 NIC stall 이슈의 실제 root cause 는 다른 곳 (`proc.sh:137` 에이전트 미설치, #287 참조) 으로 판명. v0.5.5 의 layered recovery (PodState.UNRESPONSIVE + tray auto-recovery 가 TermService cycle + power-monitor.ps1 SYSTEM task 가 Win32_PowerManagementEvent watch) 가 이미 natural NIC drop catch. proactive override 제거로 install.bat 첫 부팅이 22 줄 짧아짐 + Windows 가 Desktop 플랫폼 role override 받아들이는 가정에 더 이상 의존 안 함.
