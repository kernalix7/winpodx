...
- **Live 로그 레벨 picker** — Terminal 탭에 `DEBUG / INFO / WARNING / ERROR / CRITICAL / RAW` 드롭다운 추가. 실행 중인 logger 즉시 retarget + `cfg.logging.level` 에 persist. DEBUG 선택 시 "agent not ready" / "starting" 같은 stuck 상태 triage 에 유용한 per-tick probe / state 로그 노출; **RAW** 는 추가로 `podman logs -f` 출력 (dockur / QEMU / Windows 측 메시지) 도 같은 Terminal 에 interleave — winpodx 자체 로그로 부족할 때 한 곳에서 다 볼 수 있음.
