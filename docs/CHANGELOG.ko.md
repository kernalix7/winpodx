...
### 변경
- **Release 트리거가 `*RTM*` glob 에서 별도 `REL-` 마커 태그로 전환.** "RTM" 접미사가 원래 release 마커였지만 (`v0.3.0-RTM1`) 프로젝트가 PEP 440 / SemVer-hyphen 버전 (`0.4.0`, `0.4.0rc1`) 으로 이동하면서 RTM-suffix 태그를 더 이상 ship 안 함 — `release.yml` 의 glob 이 orphan 됨. 마커를 버전 태그 안에 섞으면 (`v0.4.0-REL` 같이) 버전 체계 자체가 오염됨 (PEP 440 valid 아님) — 새 디자인은 둘을 분리: 버전 태그는 깨끗 (`v0.4.0`, `v0.4.0rc1`), 같은 commit 에 박은 두 번째 태그 (`REL-v0.4.0`, `REL-v0.4.0rc1`) 가 release-trigger 마커 역할. `release.yml` 이 이제 `REL-*` 매치하고 prefix 떼고 버전 추출 — public Release 페이지 title 은 `v0.4.0` (not `REL-v0.4.0`), CHANGELOG lookup 은 `## [0.4.0]` 에 정확히 매치. Prerelease 감지는 PEP 440 (`0.4.0rc1`) + SemVer (`0.4.0-rc1`) 둘 다 커버. 사용 흐름: `git tag v0.4.0 && git tag REL-v0.4.0 && git push origin v0.4.0 REL-v0.4.0`. plain version tag 는 여전히 4개 packaging 워크플로우 (`.deb` / RPM / OBS / AUR) 트리거 (`v*.*.*` glob); REL 마커는 GitHub Release 페이지 전용 opt-in. 기존 v0.3.0-RTM1 시대 release 들은 그대로 유지; 트리거 변경은 미래 태그에만 적용.

### 수정
- **install.bat 가 first boot 의 PS Expand-Archive 에서 더 이상 hang 안 함 (`agent 가 설치 안 되는` 진짜 root cause).** PR #104 가 host-side cascade (migrate apply chain + discovery 가 FreeRDP fallback 으로 install.bat session kick) 는 막았지만, kernalix7 의 2026-05-04 smoke test (PR #104 merged) 에서 같은 증상 지속: `setup.log` 없음, `install.log` 가 `--- extract attempt 1` 에서 끊김 (4줄), `agent.ps1` copy 안됨, qwinsta 에 disconnected User 세션 3개. install.bat:248 의 PS Expand-Archive 호출이 자체적으로 deadlock — host-side FreeRDP 없이 install.bat 가 47분 이상 hang, 결국 migrate 의 `_probe_password_sync` (PR #104 가 gate 안 한 한 번의 FreeRDP 호출) 가 autologon session 을 kick 시켜서 실패가 표면화. v0.3.0 (OEM v12, 마지막 정상 작동) 과 diff: install.bat 코드 자체는 pre-extract 섹션 거의 동일, rdprrap zip blob hash 동일. 변한 것: (1) OEM bundle 이 6 파일 → 13+ 로 증가 (`rdprrap-activate.ps1`, `hidden-launcher.vbs`, `launch_uwp.ps1/vbs`, `agent-respawn.ps1`, `agent/agent.ps1` 추가); (2) PR #83 (v0.3.1) 에서 dockur image 가 특정 Windows 11 digest 에 pin 됨 — 그 새 빌드가 더 엄격한 Defender 실시간 정책 ship. 조합이 install.bat 의 첫 PS engine load 시점에 `C:\OEM\` 의 Defender 실시간 스캔 트리거 — `rdprrap-installer.exe` 가 `termsrv.dll` 패치 도구라 자연스럽게 PUP-flag, 거기에 옆에 staged 된 새 PS/VBS 파일들이 합쳐져서 heuristic 매치. Defender 가 파일 lock; PS Expand-Archive 가 lock 풀릴 때까지 block; install.bat 가 PS process 종료 대기; 전체 deadlock — session kick 되기 전까진 안 풀림. 3-part fix: (1) `Add-MpPreference -ExclusionPath C:\OEM,C:\winpodx -ExclusionProcess rdprrap-installer.exe` 가 install.bat 의 가장 첫 단계 (DNS / RDP / firewall 보다 앞) 가 됨 — staging dir 안의 어떤 파일도 실시간 스캔 안 됨. (2) `PS Expand-Archive` 를 Windows 내장 `tar -xf` (System32\tar.exe, Win10 1803+ ship) 로 교체 — PowerShell engine 우회 (모듈 로드 없음, .NET assembly 해석 없음, AMSI hook 없음) 해서 PS-script 분석 레이어가 추출 가로챌 수 없음. (3) `_probe_password_sync` 가 `WINPODX_REQUIRE_AGENT=1` (install.sh 가 post-install 블록 둘러싸고 export 하는 env var) 받아들여서 agent down 일 땐 probe 자체 skip — PR #104 가 빠뜨린 마지막 host-side FreeRDP 경로. `tar` 교체는 general defense in depth 도 됨: 미래에 PS 만 targeting 하고 syscall 직접 추출 안 보는 antivirus / security tool 이 추가돼도 같은 deadlock 안 일어남. OEM bundle 22 → 23. 새 테스트: `tests/test_migrate.py::TestProbePasswordSync::test_probe_skipped_under_require_agent_env_when_agent_down`.
- **Fresh install 에서 host-side FreeRDP fallback 이 install.bat 의 autologon session 을 더 이상 kick 하지 않음 (`agent 가 설치 안 되는` regression).** kernalix7 의 2026-05-02 ~ 05-04 openSUSE Tumbleweed fresh install forensics: setup.log 자체가 없음, `C:\OEM\agent.ps1` copy 안 됨, `install.log` 가 `--- extract attempt 1` 에서 끊김 (4줄), `ServiceDll` 이 `termsrv.dll` 그대로, `qwinsta` 에 disconnected `User` session 3개 누적. install.bat 가 rdprrap-extract step (line 247) 도달은 했지만 launcher / agent staging block (line ~330) 전에 죽음 — rdprrap zip extract 와 `copy /Y agent\agent.ps1` 사이 어딘가. disconnected session 누적이 결정적 증거: 각 disconnected session = inbound RDP login 이 Windows session-takeover 시킨 흔적. `fSingleSessionPerUser=0` 은 그저 registry 값일 뿐이고 rdprrap 의 `termwrap.dll` 이 실제로 load 되기 전엔 효과 없음 — install.bat 가 그 단계까지 못 갔으니 single-session 강제 상태. 그 inbound login 들의 출처는 `winpodx migrate` 와 `winpodx app refresh` — `install.sh` 가 wait-ready phase 3 에서 /health timeout (RDP up 후 ~60초) 후 호출. 둘 다 `transport.dispatch()` 통하는데, agent 안 떠 있으면 silently `FreerdpTransport` 로 fallback. FreeRDP RemoteApp 이 새 RDP login 열고, Windows 가 autologon User 를 새 RDP-Tcp session 으로 takeover, install.bat 의 cmd.exe (autologon session 의 child) 가 죽음. PR #82 (release/0.3.1) 에서 `_apply_runtime_fixes_to_existing_guest()` 의 `inst_cmp < (0, 1, 9) <= cur_cmp` gate 를 제거했을 때 들어온 regression — 0.3.1 이전엔 apply chain 이 0.1.x → 0.1.9 boundary cross 에서만 돌았기 때문에 대다수 upgrade 에서 FreeRDP fallback 이 아예 안 일어났고, fresh-install race 도 안 보였음. 3-layer fix: (1) `_apply_runtime_fixes_to_existing_guest` 가 apply chain 호출 전에 agent /health 응답을 요구; 응답 없으면 migrate 가 "Agent not yet up — Skipping runtime apply to avoid kicking the autologon RDP session" 출력하고 skip. fresh install 에서 OEM bundle 이 이미 같은 registry / service state 적용했으니 no-op; 진짜 stale OEM bundle upgrade 라면 user 가 agent 떠있을 때 `winpodx pod apply-fixes` 수동 실행. (2) `discover_apps` 가 `WINPODX_REQUIRE_AGENT=1` (env var) 보이면 FreeRDP RemoteApp fallback 대신 `DiscoveryError(kind="agent_unavailable")` raise; install.sh 가 post-install migrate + app-refresh 블록 둘러싸고 env var export 해서 install.bat-in-flight window 에만 gate 적용. (3) `wait_for_windows_responsive` 의 agent /health 예산 60s → 180s. install.bat OEM v22+ 가 first boot 에서 agent-spawn step 도달까지 60초 넘게 걸리는 게 정상 (Sysprep + DNS + RDP + firewall + rdprrap install + launcher staging) — 기존 cap 이 매 smoke test 에서 30-60초 차이로 /health miss. 세 layer 합쳐서 fresh install 동안: install.bat session 을 kick 하는 게 없으니 agent 가 깔끔하게 올라옴; 진짜 실패한 step 은 pending 으로 기록되고 다음 CLI / GUI launch 의 pending-resume 가 재시도. 새 테스트: `tests/test_migrate.py::TestApplyRuntimeFixesAgentGate` (3 cases) + `TestDiscoverAppsRequireAgent` (2 cases).
- **install.bat 가 rdprrap 의 TermService cycle 중간에서 더 이상 죽지 않음.** kernalix7 의 2026-05-03 fresh install forensics: `C:\winpodx\rdprrap\install.log` 가 깨끗하게 `FINAL: enabled` (rdprrap-activate.ps1 의 마지막 라인) 에서 끝났는데, install.bat 가 PS call 직후 작성하는 `(echo rdprrap-activate.ps1 exit=N)>>install.log` 라인이 — 안 떨어짐. setup.log (PR #90 진단) 안 만들어짐. launchers 가 `C:\Users\Public\winpodx\launchers\` 에 staged 안 됨. `HKCU\Run\WinpodxAgent` 등록 안 됨. agent inline spawn (PR #94) 안 됨. install.bat 의 rdprrap 이후 모든 섹션 silently 사라짐. 원인: dockur 의 autologon 흐름에서 Sysprep first-logon User session 이 TermService 통해 관리됨 (우린 "local console" 로 가정 — 그 가정이 틀렸음). `rdprrap-activate.ps1` 가 OEM-time cycle 의 끝에 `net stop TermService /y` 실행했을 때 autologon session 자체가 죽었고, install.bat 의 cmd.exe (그 session 의 child) 도 같이 죽음 — silently, 에러 없이. 수정: rdprrap-activate.ps1 의 TermService cycle 이 이제 OEM 모드 (synchronous, `-Detached` 없음) 에서 skip 됨; 스크립트는 registry 만 patch 하고 마커를 `patched-pending-cycle` 로 stamp. install.bat 가 실제 cycle 을 마지막 action 으로 수행 — 다른 모든 step (launchers, HKCU\Run, agent spawn, `setup_done.txt`) 이 commit 된 후. 그러면 cycle 이 install.bat session 을 죽여도 모든 setup 작업이 이미 디스크에 있음. autologon retry 가 새 user session 만들고, 거기엔 새 TermService (termwrap.dll loaded), HKCU\Run fire, agent 정상 시작, multi-session 활성. Detached/runtime 모드 (`winpodx pod multi-session on` 에서) 는 inline cycle 그대로 — wscript-detached PS 가 design 상 session death 를 살아남음. OEM 번들 21 -> 22.
