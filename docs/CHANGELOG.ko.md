...
- **Live 로그 레벨 picker** — Terminal 탭에 `DEBUG / INFO / WARNING / ERROR / CRITICAL` 드롭다운 추가. 실행 중인 logger 즉시 retarget + `cfg.logging.level` 에 persist. DEBUG 선택 시 "agent not ready" / "starting" 같은 stuck 상태 triage 에 유용한 per-tick probe / state 로그 노출.
...
- 새 `[logging]` config 섹션 + `level` 필드 (`DEBUG | INFO | WARNING | ERROR | CRITICAL`). `setup_logging()` 가 시작 시 읽음 — 모든 winpodx CLI / GUI 실행이 선택된 레벨 사용. 기본 `INFO`; 모르는 값은 reject 없이 `INFO` fallback. Terminal 탭 드롭다운이 live logger 변경 + TOML persist.
...
- `winpodx app run <slug>` 가 FreeRDP 2.x 호스트 (Ubuntu 22.04 LTS) 에서 Microsoft Store 를 띄우던 문제 수정. Win32 RemoteApp 실행 시 FreeRDP 3 의 조합 syntax `/app:program:X,name:Y,cmd:Z` 를 하나의 `/app:` argument 로 emit 하고 있었는데, FreeRDP 2.11.x 가 그 문자열을 literal program path 로 파싱, Windows 측에서 launch 실패, shell handler fallback 으로 일치하지 않는 앱 이름이 Microsoft Store 를 띄움. Win32 경로를 FreeRDP 2/3 모두 받는 separate-flag 형식 (`/app:` + `/app-name:` + `/app-cmd:`) 으로 변경. UWP 경로는 변경 없음. `default_args` 의 comma-sanitisation hack 도 더 이상 필요 없어 제거. (reported by @poetman, #158)
