...
- **AppImage 릴리스 artifact (#227).** `packaging/appimage/` 레시피 + `.github/workflows/appimage-publish.yml` CI workflow. 태그 push (`v*.*.*`) 마다 [python-appimage](https://github.com/niess/python-appimage) 로 `x86_64.AppImage` 빌드 → GitHub release 에 wheel / `.deb` / `.rpm` 와 함께 attach. Lean bundle — Python 3.11 런타임 + winpodx wheel + Qt6 (PySide6) + reverse-open extras. 호스트의 FreeRDP / Podman / KVM 은 그대로 필요; 부족하면 기존 `winpodx setup` / `winpodx doctor` 가 distro 별 설치 안내 출력. 타겟 사용자: immutable distro (Silverblue / Kinoite / Aeon / Steam Deck), `curl ... | bash` 불가 환경, 친구 노트북 빠른 시도. README + INSTALL.md (en + ko) 에 AppImage section 추가. @leandromqrs 제안.
