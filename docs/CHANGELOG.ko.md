...
### Fixed
- `winpodx app run <slug>` 가 FreeRDP 2.x 호스트 (Ubuntu 22.04 LTS) 에서 Microsoft Store 를 띄우던 문제 수정. Win32 RemoteApp 실행 시 FreeRDP 3 의 조합 syntax `/app:program:X,name:Y,cmd:Z` 를 emit 하고 있었는데, FreeRDP 2.11.x 가 이걸 mis-parse 하고 Windows 가 일치하지 않는 앱 이름에 대해 shell handler fallback 으로 Store 를 띄움. Win32 경로를 FreeRDP 2/3 모두 받는 separate-flag 형식 (`/app:` + `/app-name:` + `/app-cmd:`) 으로 변경. UWP 경로는 변경 없음. (reported by @poetman, #158)
