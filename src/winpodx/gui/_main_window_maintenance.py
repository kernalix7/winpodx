...
Holds the slot handlers driven by Maintenance-page buttons: lock-file
cleanup, Windows Update enable/disable, time sync, suspend / resume,
debloat, Windows-side runtime fixes, and "open Windows desktop". Pulled
out of ``main_window.py`` to keep that file focused on overall window
orchestration.

Host-class contract (only listed for readers; not enforced):
    info_label: QLabel              — the small status text below buttons.
    _update_status_label: QLabel    — Windows-Update status line.
    _btn_enable_updates: QPushButton
    _btn_disable_updates: QPushButton
    app_launched: Signal(str)
    app_launch_failed: Signal(str)
    pod_status_updated: Signal(str, str)
    _refresh_pod_status() -> None   — defined on the host class.
"""
